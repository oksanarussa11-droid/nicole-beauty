...
Администратор может редактировать, мягко удалять, восстанавливать и создавать
ретроактивные записи о посещениях через панель. Аутентификация — HMAC-подписанная
кука («режим правки») со сроком 15 минут.

**Обязательная переменная окружения:**
- `ADMIN_SESSION_SECRET` — случайная строка ≥16 символов. Сгенерировать:
...
Проверка реальной доставки в Telegram по подтверждённому номеру:

```sh
./scripts/validate-telegram-reminder.sh +5548991980411
```

Скрипт не читает секреты локально: запрос подписывается через Supabase Vault и
выполняется production-функцией. Если клиент ещё не запустил бота и не поделился
собственным контактом, проверка завершается с HTTP 404 без отправки сообщения.
